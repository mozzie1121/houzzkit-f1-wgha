...
static int rockchip_usb2phy_power_on(struct phy *phy)
{
	...
	/* RM01: enable the 480M clock output so the DWC3 UTMI interface has
	 * a clock. U-Boot has no clock consumer for the PHY output, so do it
	 * here explicitly (mirrors rockchip_usb2phy_clk_enable). */
	if (!property_enabled(priv->reg_base, &phy_cfg->clkout_ctl)) {
		property_enable(priv->reg_base, &phy_cfg->clkout_ctl, true);
		printf("RM01-PHY: clkout readback=%d\n",
		       property_enabled(priv->reg_base, &phy_cfg->clkout_ctl));
