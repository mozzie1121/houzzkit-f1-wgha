			      ...
			      struct dwc3_generic_priv *priv)
{
	...
	printf("RM01-DWC3: phy_setup rc=%d phys=%u\n", rc, priv->phys.count);
	if (rc && rc != -ENOTSUPP)
		return rc;
	...
	rc =  dwc3_init(dwc3);
	printf("RM01-DWC3: dwc3_init rc=%d\n", rc);
	if (rc) {
...
int dwc3_glue_probe(struct udevice *dev)
{
	...
	printf("RM01-GLUE: clk_init ret=%d clks=%u\n", ret, glue->clks.count);
	if (ret)
		return ret;

	ret = dwc3_glue_reset_init(dev, glue);
	printf("RM01-GLUE: reset_init ret=%d resets=%u\n", ret, glue->resets.count);
	if (ret)
		return ret;
